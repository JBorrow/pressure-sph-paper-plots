...
from energy_injection_just_like_EAGLE import *
...
#    true_entropies = np.random.rand(positions.shape[0])
    true_entropies = np.ones(len(positions))
    ...
    for zorder, (energy_ratio, target_injection) in enumerate(zip(energy_ratios, target_injections)):
        ax.plot(
            range(len(energy_ratio))[1:],
            energy_ratio[1:],
    ...
    fig.savefig("energy_ratio_EAGLE.pdf")

    print("Saved plot to energy_ratio_EAGLE.pdf")
